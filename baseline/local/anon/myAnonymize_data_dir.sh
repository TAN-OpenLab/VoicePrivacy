...
if [ $stage -le 1 ]; then
  ...
  local/anon/myMake_pseudospeaker.sh --rand-level ${pseudo_xvec_rand_level} \
...
if [ $stage -le 4 ]; then
  ...
  local/anon/make_netcdf.sh --stage 0 data/${data_dir} ${ppg_dir}/ppg_${data_dir}/phone_post.scp \
	  ${anon_xvec_out_dir}/xvectors_${data_dir}/myLshPseudo_xvecs/EachOne_xvecs/pseudo_EachOne_xvector.scp \
...
if [ $stage -le 7 ]; then
  ...
  wav_path=${data_netcdf}/${data_dir}/myLshNsf_output_wav
  new_data_dir=data/${data_dir}${anon_data_suffix}_lsh
  ...
  cp ${anon_xvec_out_dir}/xvectors_${data_dir}/myLshPseudo_xvecs/spk2gender ${new_data_dir}/
